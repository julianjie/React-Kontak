...
const [error, setError] = useState(null); // Menginisialisasi state 'error' untuk menyimpan pesan error jika ada
    ...
    useEffect(() => {
        axios
            .get(`https://latihan-uts-pw2.vercel.app/api/api/kontak/show/${id}`) // Mengirimkan request GET untuk mendapatkan data fakultas berdasarkan ID
            .then((response) => {
    ...
    const handleSubmit = (e) => {
        ...
        axios
            .patch(`https://latihan-uts-pw2.vercel.app/api/api/kontak/${id}`, { nama }) // Mengirimkan request PATCH untuk mengupdate data fakultas berdasarkan ID
        .then((response) => {
            navigate("/kontak"); // Jika update berhasil, navigasi kembali ke halaman list fakultas
        })
        .catch((error) => {
    ...
    return(
        <div>
        <h2>Edit Kontak</h2> {/* Menampilkan judul halaman */}
        ...
        <div className="mb-3">
            <label htmlFor="nama" className="form-label">Nama Kontak</label> {/* Label untuk input nama */}
            <input
                type="text"
